	...
	)
{
	...
	{
		if(SC.ENVIRONMENT.BuildTimeInf && SC.ENVIRONMENT.BuildTimeInf.isSCLite)
		{
			if (SC.CONFIGURATION.unmanagedResourcesFolderName) 
			{
				return getAbsoluteUrl('site/' + SC.CONFIGURATION.unmanagedResourcesFolderName + '/' + file);
			}
			else
			{
				return  getAbsoluteUrl('default/' + file);
			}			
		}
